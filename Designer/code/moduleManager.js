...
/* 
THIS DOES NOT WORK
function renumberEpisodes(mName){
	var dummy = new Dict();
	var thisModule = m.get(mName);
	var moduleKeys = thisModule.getkeys();
	post(moduleKeys);
	for (ep = 0; ep > moduleKeys.length; ep ++){
		dummy.replace(ep, thisModule.get(moduleKeys[ep]));
		}
	m.set(mName, dummy.clone());
	}
